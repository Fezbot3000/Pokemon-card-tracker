...
// New function: List backup files for a user
exports.listBackupFiles = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    // List all files in the user's backup directory
    const [files] = await bucket.getFiles({ prefix: backupPath });
    
    // Format the files list for client consumption
    const filesList = files.map(file => {
      // Remove the path prefix to get just the filename
      const name = file.name.replace(backupPath, '');
      return {
        name,
        path: file.name,
    ...
    functions.logger.info(`Listed ${filesList.length} backup files for user ${userId}`);
...
// New function: Get backup file content
exports.getBackupFileContent = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const [exists] = await file.exists();
    if (!exists) {
      throw new functions.https.HttpsError('not-found', `File ${fileName} not found in user's backup`);
    }
    
    // Download the file content
    const [fileContent] = await file.download();
    
    // For text files like JSON, return as string

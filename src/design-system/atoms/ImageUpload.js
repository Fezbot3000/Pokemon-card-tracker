...
const ImageUpload = ({
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      if (file.type.match('image.*')) {
        if (typeof onImageChange === 'function') {
          ...
          LoggingService.warn('ImageUpload: onImageChange prop is not a function');
        }
      }
    }
  };
  ...
  if (loadingState === 'loading') {
    return (
      <div
        className={`flex size-full items-center justify-center rounded-lg bg-gray-50 dark:bg-[#1A1A1A] ${className}`}
      >
  ...
  if (loadingState === 'error') {
    return (
      <div
        className={`flex size-full flex-col items-center justify-center rounded-lg bg-gray-50 dark:bg-[#1A1A1A] ${className}`}
      >
  ...
  return (
    <div
      className={`flex size-full flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 dark:border-gray-600 dark:bg-[#1A1A1A] ${isDragging ? 'border-[#E6185C] dark:border-[#E6185C]' : ''} transition-colors duration-200 ${className}`}
      onDragEnter={handleDragEnter}
      onDragLeave={handleDragLeave}
      onDragOver={handleDragOver}
      onDrop={handleDrop}

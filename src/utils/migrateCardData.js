...
export async function migrateCardData() {
  ...
  try {
    ...
    logger.info(
      `Processing in ${batches.length} batches of up to ${BATCH_SIZE} documents each`
    );
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      for (const document of batch) {
        ...
        if (updatedData.card && typeof updatedData.card === 'string') {
          ...
          if (words.length > 3) {
            ...
            needsUpdate = true;
            logger.debug(
              `Simplified card name for ${cardId}: "${cardData.card}" -> "${simplifiedName}"`
            );
          }
        }
        ...
        for (const requiredField of REQUIRED_FIELDS) {
          if (!updatedData[requiredField]) {
            ...
            if (!updatedData[requiredField]) {
              logger.warn(
                `Missing required field ${requiredField} for card ${cardId} and couldn't derive it`
              );
            }
          }
        }
        ...
        if (needsUpdate) {
          ...
          logger.debug(
            `Updated document ${cardId} (removed ${fieldsRemovedCount} fields)`
          );
        } else {
          stats.docsSkipped++;
        }
      ...
      if (!DRY_RUN && batchUpdates > 0) {
        ...
        logger.info(
          `Committed batch ${batchIndex + 1} with ${batchUpdates} updates`
        );
      } else if (DRY_RUN) {
        logger.info(
          `DRY RUN: Would have updated ${batchUpdates} documents in batch ${batchIndex + 1}`
        );
      }
    }
...
async function migrateSoldItems(userId, stats) {
  ...
  try {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      const batch = batches[batchIndex];
      logger.info(
        `Processing sold items batch ${batchIndex + 1} of ${batches.length}`
      );
      ...
      if (!DRY_RUN && batchUpdates > 0) {
        await writeBatchObj.commit();
        logger.info(
          `Committed sold items batch ${batchIndex + 1} with ${batchUpdates} updates`
        );
      } else if (DRY_RUN) {
        logger.info(
          `DRY RUN: Would have updated ${batchUpdates} sold items in batch ${batchIndex + 1}`
        );
      }
    }

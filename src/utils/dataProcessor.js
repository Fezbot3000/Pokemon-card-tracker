...
export const parseMultipleCSVFiles = async files => {
  ...
  if (errors.length > 0 && results.length === 0) {
    throw new Error(
      `Failed to parse CSV files: ${errors.map(e => `${e.file}: ${e.error}`).join(', ')}`
    );
  }
...
export const processImportedData = (
  ...
  return existingCardsCopy.map(card => {
    const importedCard = importedData.find(
      ic => ic['Slab Serial #']?.toString() === card.slabSerial?.toString()
    );
    ...
    if (importedCard) {
      ...
      if (importedCard['Current Value'] !== undefined) {
        const newValueInSourceCurrency =
          parseFloat(importedCard['Current Value']) || 0;
        ...
        if (updateExistingValues || !updatedCard.currentValueAUD) {
          ...
          updatedCard.potentialProfit = Number(
            (
              updatedCard.currentValueAUD - (updatedCard.investmentAUD || 0)
            ).toFixed(2)
          );
        }
      }
      ...
      if (fillMissingFields) {
        ...
        Object.entries(fieldMappings).forEach(([csvField, cardField]) => {
          if (
            importedCard[csvField] !== undefined &&
            importedCard[csvField] !== null
          ) {
            // Only update if we should update existing values or the field is empty
 ...
 */
export const processMultipleCollectionsUpdate = (
  importedData,
  allCollections,
  options = {}
) => {
  ...
  // Default options
  const { fillMissingFields = true, updateExistingValues = true } = options;
    ...
    const updatedCollection = collection.map(card => {
      ...
      if (importedCard) {
        ...
        if (importedCard['Current Value'] !== undefined) {
          ...
          // Only update if we should update existing values or the field is empty
          if (updateExistingValues || !updatedCard.currentValueAUD) {
            updatedCard.currentValueAUD = convertToAUD(
              newValueInSourceCurrency
            );
        ...
        if (fillMissingFields) {
          ...
          const fieldMappings = [
            ...
            {
              csvField: 'Population',
              cardField: 'population',
              statField: 'population',
            },
            {
              csvField: 'Date Purchased',
              cardField: 'datePurchased',
              statField: 'datePurchased',
            },
          ];
...
export const calculateCardMetrics = cards => {
  const totalInvestment = Number(
    cards.reduce((sum, card) => sum + (card.investmentAUD || 0), 0).toFixed(2)
  );
  const totalValue = Number(
    cards.reduce((sum, card) => sum + (card.currentValueAUD || 0), 0).toFixed(2)
  );
  const totalProfit = Number((totalValue - totalInvestment).toFixed(2));
  const profitPercentage = Number(
    (totalInvestment > 0 ? (totalProfit / totalInvestment) * 100 : 0).toFixed(2)
  );

  // Count cards with profit vs loss
  const profitableCards = cards.filter(
    card => (card.currentValueAUD || 0) > (card.investmentAUD || 0)
  ).length;

  const unprofitableCards = cards.filter(
    card => (card.currentValueAUD || 0) < (card.investmentAUD || 0)
  ).length;

...
export const useViewCache = () => {
  ...
  const switchView = (viewName) => {
    ...
    if (viewsRef.current[activeView]) {
      ...
      CacheManager.saveScrollPosition(activeView, scrollPositionsRef.current[activeView]);
    }
    ...
    setTimeout(() => {
      if (viewsRef.current[viewName]) {
        const savedPosition = scrollPositionsRef.current[viewName] || 
                             CacheManager.getScrollPosition(viewName) || 0;

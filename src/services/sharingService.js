...
class SharingService {
  ...
  findBestCardImage(cards) {
    ...
    const sortedByValue = cardsWithImages.sort((a, b) => {
      const valueA =
        a.originalCurrentValueAmount ||
        a.currentValueAUD ||
        a.currentValue ||
        0;
      const valueB =
        b.originalCurrentValueAmount ||
        b.currentValueAUD ||
        b.currentValue ||
        0;
  ...
  generateMetaTags(shareData, cards = null) {
    ...
    return {
      ...
      description:
        ...
        `View ${shareData.ownerName}'s trading card collection with ${cards ? cards.length : 'amazing'} cards`,
      url: shareUrl,
      image: shareImage,
      type: 'website',
      siteName: 'Collectr',
  ...
  async shareCollection(shareData) {
    ...
    const shareContent = {
      ...
      text:
        shareData.description ||
        `Check out ${shareData.ownerName}'s trading card collection`,
      url: shareUrl,
    };
  ...
  formatCollectionStats(cards) {
    ...
    const gradedCards = cards.filter(
      card => card.gradingCompany || card.gradeCompany
    ).length;
    const categories = [
  ...
  filterAndSortCards(cards, filters = {}) {
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter(
        card =>
          (card.cardName || card.name || card.card)
            ?.toLowerCase()
            .includes(searchTerm) ||
    ...
    if (filters.sortBy) {
      filtered.sort((a, b) => {
        switch (filters.sortBy) {
          case 'name':
            return (a.cardName || a.name || a.card || '').localeCompare(
              b.cardName || b.name || b.card || ''
            );
          case 'set':
            return (a.set || a.cardSet || '').localeCompare(
              b.set || b.cardSet || ''
            );
          case 'year':

...
class CloudSyncService {
  ...
  async incrementalBackup(userId, setProgress, setStatus, addLog) {
    try {
      ...
      const metadataFile = listResult.items.find(
      ...
      if (metadataFile) {
        ...
        if (metadataResponse.ok) {
          ...
          addLog(
            `Cloud backup metadata: Timestamp ${existingMetadata.timestamp}, Collections: ${existingMetadata.collectionCount}, Images: ${existingMetadata.imageCount}`
          );
        }

        // Download collections data
        const collectionsFile = listResult.items.find(
          item => item.name === 'collections.json'
        );
        if (collectionsFile) {
          ...
          if (collectionsResponse.ok) {
            ...
            addLog(
              `Downloaded cloud collections: ${cloudCollections.length} collections.`
            );
          }
        }
        ...
        try {
          ...
          // If collection is an object with an id property, try to fetch cards from CardRepository
            ...
            try {
              ...
              addLog(
                `Error fetching cards from repository for collection ${collection.id}: ${repoError.message}`
              );
            }
          }
        } catch (error) {
      ...
      if (allCardIds.length === 0) {
        try {
          ...
          addLog(
            `Extracted ${allCardIds.length} card IDs from collections object.`
          );
        } catch (error) {
          addLog(
            `Error extracting card IDs from collections object: ${error.message}`
          );
        }
      }

      addLog(
        `Found ${allCardIds.length} valid card IDs across all collections.`
      );
      ...
      if (validLocalImages.length < localImages.length) {
        addLog(
          `Found ${localImages.length - validLocalImages.length} orphaned images that will be excluded from backup.`
        );
      }

      // Find images to add (valid images in local but not in cloud)
      const imagesToAdd = validLocalImages.filter(
        image => !cloudImageIds.includes(image.id)
      );
      ...
      const imagesToRemove = cloudImageIds.filter(
        id => !validLocalImageIds.includes(id)
      );
      ...
      const collectionsBlob = new Blob([JSON.stringify(collectionsToUpdate)], {
        type: 'application/json',
      });
      ...
      if (imagesToAdd.length > 0) {
        ...
        for (let i = 0; i < imagesToAdd.length; i++) {
          ...
          try {
            ...
            const imageRef = ref(
              storage,
              `backups/${userId}/images/${imageName}`
            );
            ...
            if (image.blob instanceof Blob) {
              ...
              try {
                ...
                addLog(
                  `Error converting base64 for image ${image.id}: ${conversionError.message}`
                );
                continue;
              }
            } else {
              ...
              if (image.blob) {
                addLog(
                  `Creating blob from non-Blob blob property for image ${image.id}`
                );
                imageBlob = new Blob([image.blob], { type: 'image/png' });
              } else if (image.data) {
                addLog(
                  `Creating blob from non-Blob data property for image ${image.id}`
                );
            ...
            // Upload the image
            addLog(
              `Uploading image ${i + 1}/${imagesToAdd.length}: ${imageName}`
            );
        ...
        addLog(
          `New images uploaded: ${successfulUploads}/${imagesToAdd.length}`
        );
      } else {
      ...
      if (imagesToRemove.length > 0) {
        addLog(
          `Removing ${imagesToRemove.length} deleted images from cloud...`
        );
        ...
        for (let i = 0; i < imagesToRemove.length; i++) {
          ...
          const progress =
            80 + Math.round(((i + 1) / imagesToRemove.length) * 15); // Deletions take 15% of progress
          setProgress(progress);
        ...
        addLog(
          `Deleted images removed: ${successfulDeletions}/${imagesToRemove.length}`
        );
      } else {

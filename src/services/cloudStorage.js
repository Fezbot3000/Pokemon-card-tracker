...
export const saveImageToCloud = async (
  ...
  try {
    ...
    try {
      ...
      logger.debug(
        `Successfully requested deletion of existing image for card ${cardId}`
      );
    } catch (deleteError) {
      // Don't fail if deletion fails, just log it
      logger.warn(
        `Failed to delete existing image for card ${cardId}:`,
        deleteError
      );
    }
    ...
    logger.debug(
      `Image upload successful, URL: ${cacheBustUrl.substring(0, 30)}...`
    );
    return cacheBustUrl;
  } catch (error) {

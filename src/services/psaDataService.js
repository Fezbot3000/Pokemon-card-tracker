...
class PSADataService {
  ...
  async getCardFromCache(certNumber) {
    try {
      ...
      logger.error(
        `Error fetching PSA card from Firestore: ${error.message}`,
        error
      );
      return null;
    }
  ...
  async saveCardToCache(certNumber, cardData) {
    try {
      ...
      if (!hasPermission) {
        logger.debug(
          `Skipping PSA cache write for cert #${certNumber} - user not authenticated`
        );
      ...
      if (error.code === 'permission-denied') {
        logger.debug(
          'Permission denied saving to PSA database - user likely not authenticated'
        );
        return false;
      }

      logger.error(
        `Error saving PSA card to Firestore: ${error.message}`,
        error
      );
      return false;
    }
  ...
  async initializeCollection() {
    ...
    logger.debug(
      'PSA collection initialization skipped - will be handled on-demand'
    );
    return true;
  }
}

...
const fetchCardPricingDirect = async (
  cardName,
  setName = null,
  cardNumber = null
) => {
...
const fetchCardPricing = async (
  ...
  try {
    logger.debug(
      `Fetching pricing for card: ${cardName}, set: ${setName}, number: ${cardNumber}`
    );
    ...
    try {
      ...
      if (result.data && result.data.success) {
        ...
        logger.info(
          `Successfully fetched pricing for ${cardName} via Firebase Function`
        );
        return pricingData;
      } else {
      ...
      const directResult = await fetchCardPricingDirect(
        cardName,
        setName,
        cardNumber
      );
      ...
      logger.info(
        `Successfully fetched pricing for ${cardName} via direct API call`
      );
      return directResult;
    }
  } catch (error) {
...
export { fetchCardPricing, extractBestPrice, formatPricingForDisplay };

...
import shadowSync from './services/shadowSync'; // Import shadowSync service
...
3. All your data will be restored including collections, cards, sold items, purchase invoices, and user preferences`;
  ...
  // Handle resetting all data - comprehensive cleanup of local and cloud data
    ...
    try {
      ...
      if (user) {
        try {
          ...
          // First attempt with the enhanced repository method
          await repository.deleteAllUserData();
          
          // Verify cards are actually deleted from Firestore
          try {
            updateProgress('Verifying card deletion');
            // Direct check of the cards collection
            const verifyCardsRef = collection(firestoreDb, 'users', user.uid, 'cards');
            const verifySnapshot = await getDocs(verifyCardsRef);
            
            if (!verifySnapshot.empty) {
              // If cards still exist, try direct deletion as a fallback
              logger.warn(`Found ${verifySnapshot.size} cards still in Firestore, forcing direct deletion`);
              updateProgress(`Forcing deletion of ${verifySnapshot.size} remaining cards`);
            ...
            // Continue with the reset process despite errors
          }
          
          // 4. Additional cleanup for purchase invoices if they exist
          try {
            updateProgress('Removing purchase invoices');
            const purchaseInvoicesRef = collection(firestoreDb, 'users', user.uid, 'purchase-invoices');
            ...
            if (!invoicesSnapshot.empty) {
              ...
              if (operationCount > 0) {
                await batch.commit();
                logger.debug(`Deleted ${operationCount} purchase invoices from Firestore`);
              }
            }
          } catch (invoiceError) {
          ...
          try {
            ...
            const userPrefsRef = doc(firestoreDb, 'users', user.uid, 'preferences', 'app');
      ...
      try {
        // First, explicitly delete all cards to ensure they're removed
        await db.clearCards();
        // Then reset all other data
      ...
      // Reset application state - only use functions that exist
      updateProgress('Resetting application state');
      setCollections({}); // Reset collections to empty
      setSelectedCollection('All Cards'); // Reset to default collection
      ...
      toast.success('Reset complete - reloading application', { id: 'reset-data', duration: 3000 });

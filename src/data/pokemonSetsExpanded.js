...
const POKEMON_SETS = {
  // Base Era
  1996: [{ value: 'Base Set (JP)', label: 'Base Set (JP)' }],
  1997: [
  ...
  2009: [
    ...
    {
      value: 'HeartGold & SoulSilver Series (JP)',
      label: 'HeartGold & SoulSilver Series (JP)',
    },
  ],
  2010: [
  ...
  2017: [
    ...
    {
      value: 'Ultra Sun & Ultra Moon (JP)',
      label: 'Ultra Sun & Ultra Moon (JP)',
    },
...
const getPokemonSetsByYear = year => {
  ...
  const standardSets = POKEMON_SETS[year]
    ? POKEMON_SETS[year].map(set => set.value)
    : [];
  const customSets = userCustomSets[year]
    ? userCustomSets[year].map(set => set.value)
    : [];

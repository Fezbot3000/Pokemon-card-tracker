...
const CloudSync = ({ onExportData, onImportCollection }) => {
  ...
  const handleSyncToCloud = async () => {
    ...
    try {
      ...
      toast.success('Backup completed successfully! (100%)', {
        id: 'cloud-backup',
      });
    } catch (error) {
      ...
      if (error.code === 'storage/retry-limit-exceeded') {
        toast.error(
          'Backup file too large for mobile. Try on desktop or reduce collection size.',
          { id: 'cloud-backup' }
        );
  ...
  const handleImportFromCloud = async () => {
    ...
    try {
      ...
      try {
        ...
        setSyncProgress(20);
        toast.loading('Found backup, starting download... (20%)', {
          id: 'cloud-restore',
        });
        ...
        xhr.onprogress = event => {
          if (event.lengthComputable) {
            ...
            toast.loading(`Downloading backup... (${percentRounded}%)`, {
              id: 'cloud-restore',
            });
          }
        };
        ...
        xhr.onload = function () {
          if (this.status === 200) {
            ...
            setSyncProgress(70);
            toast.loading(`Processing backup... (70%)`, {
              id: 'cloud-restore',
            });
            ...
            onImportCollection(file, {
              ...
              onProgress: (step, percent, message) => {
                ...
                toast.loading(
                  `${message || `Step ${step}: ${Math.round(scaledPercent)}%`}`,
                  { id: 'cloud-restore' }
                );
              },
            })
              .then(() => {
                setSyncProgress(100);
                toast.success('Restore completed successfully! (100%)', {
                  id: 'cloud-restore',
                });
        ...
        if (error.code === 'storage/object-not-found') {
          toast.error('No backup found in cloud storage', {
            id: 'cloud-restore',
          });
        } else {
      ...
      {(isSyncing || isImporting) && (
        <div className="mt-4 w-full">
          ...
          <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            ...
            {syncProgress >= 50 &&
              syncProgress < 80 &&
              (isSyncing ? 'Uploading files...' : 'Downloading files...')}

...
const OptimizedView = ({ 
  ...
  useEffect(() => {
    ...
    if (isActive || !hasRendered.current || forceRefresh) {
      ...
      logger.debug(`OptimizedView: Rendering ${viewName}, active:${isActive}, forceRefresh:${forceRefresh}`);
    }
    ...
    if (isActive && viewRef.current && saveScrollPosition) {
      ...
      setTimeout(() => {
        if (viewRef.current) {
          ...
          logger.debug(`OptimizedView: Restored scroll position for ${viewName}: ${savedPosition}px`);
        }
      }, 50);
    }
  ...
  useEffect(() => {
    if (!isActive && viewRef.current && saveScrollPosition) {
      ...
      logger.debug(`OptimizedView: Saved scroll position for ${viewName}: ${currentPosition}px`);
    }
  }, [isActive, viewName, saveScrollPosition]);

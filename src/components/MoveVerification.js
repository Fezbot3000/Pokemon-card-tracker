...
const MoveVerification = () => {
    ...
    try {
      ...
      if (lastMoveData && lastMoveData.cards && lastMoveData.cards.length > 0) {
        ...
        if (collections[targetCollection]) {
          ...
          for (const movedCard of lastMoveData.cards) {
            ...
            if (movedCard.from !== 'Unknown Collection' && collections[movedCard.from]) {
      ...
      if (lastChangeData) {
        ...
        if (collections[targetCollection]) {
          ...
          // Check if card is removed from source collection
          let removedFromSource = true;
          if (lastChangeData.from !== 'Unknown Collection' && collections[lastChangeData.from]) {
      ...
      if (featureFlags.enableFirestoreSync && user) {
        ...
        if (lastMoveData && lastMoveData.cards && lastMoveData.cards.length > 0) {
          ...
          for (const movedCard of lastMoveData.cards) {
            try {
              ...
              const foundInTargetCollection = cloudCard && 
              ...
              console.error(`Error verifying card ${movedCard.id} in cloud:`, error);
              results.cloudVerification.details.push({
                cardId: movedCard.id,
                cardName: movedCard.name,
                error: error.message,
        ...
        if (lastChangeData) {
          try {
            ...
            console.error(`Error verifying card ${lastChangeData.cardId} in cloud:`, error);
            results.cloudVerification.details.push({
              cardId: lastChangeData.cardId,
              cardName: lastChangeData.cardName,
              error: error.message,
        ...
        results.cloudVerification.details.push({
          message: 'Cloud verification skipped - feature flag disabled or user not logged in'
        });
      }
  ...
  return (
    <div className="mb-4 mt-16 rounded-lg bg-white p-4 shadow dark:bg-[#1B2131] sm:mt-20">
      ...
      {lastMoveData && (
        <div className="mb-4">
          <h3 className="font-medium text-gray-900 dark:text-white">Batch Move</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Moved {lastMoveData.cards.length} card(s) to "{lastMoveData.targetCollection}" at {new Date(lastMoveData.timestamp).toLocaleString()}
          </p>
      ...
      {lastChangeData && (
        <div className="mb-4">
          <h3 className="font-medium text-gray-900 dark:text-white">Collection Dropdown Change</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Moved "{lastChangeData.cardName}" from "{lastChangeData.from}" to "{lastChangeData.to}" at {new Date(lastChangeData.timestamp).toLocaleString()}
          </p>
        </div>
      )}
        ...
        <div className="mt-4 border-t border-gray-200 pt-4 dark:border-gray-700">
          ...
          <div className="mb-3">
            ...
            <ul className="mt-1 text-sm">
              {verificationResults.localVerification.details.map((detail, index) => (
                <li key={index} className={`${detail.success ? 'text-green-500' : 'text-red-500'}`}>
                  • {detail.cardName}: {detail.success ? 'Correctly moved' : 'Move failed'} 
                  {!detail.success && (
                    <span>
                      {!detail.foundInTarget && ' (Not found in target collection)'}
                      {!detail.removedFromSource && ' (Not removed from source collection)'}
                    </span>
                  )}
                </li>
              ))}
          ...
          <div className="mb-3">
            ...
            <ul className="mt-1 text-sm">
              {verificationResults.cloudVerification.details.map((detail, index) => (
                <li key={index} className={`${detail.success ? 'text-green-500' : detail.message ? 'text-gray-500' : 'text-red-500'}`}>
                  {detail.message ? 
                    `• ${detail.message}` : 

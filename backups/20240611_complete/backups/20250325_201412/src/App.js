...
function AppContent({ user }) {
  ...
  // Improve automatic image sync when collections first load
  useEffect(() => {
    if (user && !isLoading && Object.keys(collections).length > 0) {
      ...
      if (hasCards) {
        // Get the timestamp of the last automatic image sync
        const lastImageSync = localStorage.getItem('lastImageSync');
        const currentTime = Date.now();
        // Sync every 6 hours instead of daily for more frequent syncing
        const sixHoursMs = 6 * 60 * 60 * 1000;
        
        // Run sync if:
        // 1. Never synced before
        // 2. Last sync was more than 6 hours ago
        // 3. We have cards but no images yet
        const shouldSync = !lastImageSync || 
                          (currentTime - parseInt(lastImageSync)) > sixHoursMs;
        
        if (shouldSync) {
          console.log('Starting automatic image sync...');
          
          // Create a toast notification for the sync process
          const syncToast = document.createElement('div');
          syncToast.className = 'fixed right-4 bottom-20 z-[9999] px-6 py-3 rounded-lg shadow-lg bg-blue-500 text-white transition-all duration-300 toast-notification';
          syncToast.innerHTML = `
            <div class="flex items-center">
              <div class="mr-2 animate-spin"><span class="material-icons">sync</span></div>
              <div>Syncing images...</div>
            </div>
          `;
          ...
          db.syncImagesFromStorage(user.uid)
            .then(result => {
              // Store the sync timestamp regardless of result
              localStorage.setItem('lastImageSync', currentTime.toString());
              
              // Update toast based on result status
              if (result.status === 'complete' && result.synced > 0) {
                // Success with images synced
                syncToast.className = 'fixed right-4 bottom-20 z-[9999] px-6 py-3 rounded-lg shadow-lg bg-green-500 text-white transition-all duration-300 toast-notification';
                syncToast.innerHTML = `
                  <div class="flex items-center">
                    <div class="mr-2"><span class="material-icons">check_circle</span></div>
                    <div>Image sync complete: ${result.synced} of ${result.total} images loaded</div>
                  </div>
                `;
              } else if (result.status === 'partial') {
                // Partial success
                syncToast.className = 'fixed right-4 bottom-20 z-[9999] px-6 py-3 rounded-lg shadow-lg bg-yellow-500 text-white transition-all duration-300 toast-notification';
                syncToast.innerHTML = `
                  <div class="flex items-center">
                    <div class="mr-2"><span class="material-icons">warning</span></div>
                    <div>Partial sync: ${result.synced} of ${result.total} images loaded (${result.failed} failed)</div>
                  </div>
                `;
              } else if (result.status === 'complete' && result.synced === 0) {
                // All images already synced
                syncToast.className = 'fixed right-4 bottom-20 z-[9999] px-6 py-3 rounded-lg shadow-lg bg-gray-500 text-white transition-all duration-300 toast-notification';
                syncToast.innerHTML = `
                  <div class="flex items-center">
                    <div class="mr-2"><span class="material-icons">info</span></div>
                    <div>All images already synchronized (${result.total} total)</div>
                  </div>
                `;
              } else {
                // No images could be synced
                syncToast.className = 'fixed right-4 bottom-20 z-[9999] px-6 py-3 rounded-lg shadow-lg bg-yellow-500 text-white transition-all duration-300 toast-notification';
                syncToast.innerHTML = `
                  <div class="flex items-center">
                    <div class="mr-2"><span class="material-icons">warning</span></div>
                    <div>Could not sync any images. Will retry later.</div>
                  </div>
                `;
              }
              ...
              setTimeout(() => {
                ...
                setTimeout(() => {
                  if (document.body.contains(syncToast)) {
            ...
            .catch(error => {
              ...
              // Show error toast
              syncToast.className = 'fixed right-4 bottom-20 z-[9999] px-6 py-3 rounded-lg shadow-lg bg-red-500 text-white transition-all duration-300 toast-notification';
              syncToast.innerHTML = `
                <div class="flex items-center">
                  <div class="mr-2"><span class="material-icons">error</span></div>
                  <div>Error syncing images: ${error.message || 'Unknown error'}</div>
                </div>
              `;
              ...
              // On failure, set a shorter timeout for next sync attempt
              localStorage.setItem('lastImageSync', (currentTime - sixHoursMs + 10 * 60 * 1000).toString()); // Try again in 10 minutes
            });
        } else {

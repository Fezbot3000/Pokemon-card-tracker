...
class DatabaseService {
  ...
  syncImagesFromStorage = async (userId) => {
    if (!userId) {
      ...
      return { synced: 0, total: 0, error: 'No user ID provided' };
    }
    ...
    try {
      ...
      if (serialsToSync.length === 0) {
        ...
        return { synced: 0, total: uniqueSerials.length, status: 'complete' };
      }
          ...
          while (retries <= MAX_RETRIES && !success) {
            try {
              ...
              if (retries > MAX_RETRIES) {
                console.warn(`Could not sync image for card ${serial} after ${MAX_RETRIES} attempts:`, error);
                failedCount++;
                failedSerials.push(serial);
              } else {
                console.log(`Retry ${retries}/${MAX_RETRIES} for card ${serial}...`);
      ...
      if (syncedCount > 0) {
        const syncResult = {
          ...
          message: `Successfully synchronized ${syncedCount} of ${serialsToSync.length} images from Firebase Storage`
        };
        ...
        return {
          ...
          failed: failedCount,
          status: 'failed',
          message: 'No images could be synchronized'
        };
      }
    } catch (error) {
      ...
      return { 
        ...
        message: 'Failed to synchronize images'
      };
    }
  };
